<script setup lang="ts">
  ...
  const emits = defineEmits<{
    (e: 'back'): void,
    (e: 'toDetailChapter', val: any): void
  }>()

  const toDetailBaca = (data: any) => {
      ...
      <v-card-title>
        <v-row>
          <v-col cols="12" sm="10" class="headline text-primary pb-2 limit-text">
            <v-icon color="primary" @click="emits('back')" class="mb-1 back-to-list">mdi-arrow-left-bold</v-icon> <b>{{ data.title1 }}</b>
          </v-col>
      ...
      <v-card-text class="py-0 px-3">
        <div class="w-100">
          <v-row class="bg-primary header-episode mb-1">
            <v-col cols="6">Chapter</v-col>
            <v-col cols="6">Tanggal Rilis</v-col>
          </v-row>

          <v-row v-for="(item, i) in data.listCp" :key="i" :class="i % 2 === 0 ? 'eps-genap' : 'eps-ganjil'" @click="toDetailBaca(item)">
            <v-col cols="6">{{ item.chapter }}</v-col>
